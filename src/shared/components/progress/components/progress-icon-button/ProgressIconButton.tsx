...
export const ProgressIconButton: React.FC<ProgressIconButtonProps> = ({ className, size, icon, ...props }) => {
	const styles = progressIconButtonVariants();
	return (
		<IconButton
			icon={icon}
			variant="text"
			className={styles.base({ className })}
			iconClassName={styles.icon({ size })}
			size={size}
			{...props}
		/>
	);
